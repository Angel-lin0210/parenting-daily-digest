...
function loadKeywordsConfig() {
    ...
    if (fs.existsSync(configPath)) {
        ...
        console.log(`📋 設定檔: ${config.description || '親子教育觀察'}`);
    ...
    return [
        {
            ...
            name: '兒童發展',
            icon: '🧒',
            keywords: ['兒童發展 里程碑', '幼兒 認知發展', '孩子 語言發展']
        },
        {
            id: 'psychology',
            name: '兒童心理',
            icon: '💭',
            keywords: ['兒童心理學', '孩子 情緒管理', '依附關係 理論']
        },
        {
            id: 'parenting',
            name: '教養方法',
            icon: '👪',
            keywords: ['正向管教', '親子溝通', '設定界線']
        },
        {
            id: 'resources',
            name: '資源與經驗',
            icon: '📖',
            keywords: ['親子天下', 'PTT 親子板', '兒童心理 書籍']
        }
    ];
}
    ...
    try {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (cleanParts.length >= 2) {
                const topic = cleanParts[0].toLowerCase();
        ...
        console.log(`   🧒 兒童發展: ${manualData.development.length} 筆`);
        console.log(`   💭 兒童心理: ${manualData.psychology.length} 筆`);
        console.log(`   👪 教養方法: ${manualData.parenting.length} 筆`);
        console.log(`   📖 資源與經驗: ${manualData.resources.length} 筆`);
        console.log('');
...
async function analyzeNewsWithClaude(topic, newsArticles, manualData) {
    ...
    const prompt = `你是親子教育與兒童心理專家。請分析「${topic.name}」這個主題的重要趨勢。
...
請提供:
1. 摘要 (150字內，整合最新資訊、研究發現和實務經驗)
2. 5個關鍵重點 (每個50字內，必須基於提供的新聞和資料，包含具體案例或建議)
3. 5個延伸討論問題
...
async function fetchRealDigestData() {
    ...
    const manualData = await loadManualDataFromSheets();
    const results = {};
...
<tr><td style="background:linear-gradient(135deg,#FF6B9D 0%,#C06C84 100%);padding:32px 24px;text-align:center;">
<h1 style="margin:0;font-size:24px;color:#ffffff;">👨‍👩‍👧‍👦 親子教育每日觀察</h1>
<p style="margin:8px 0 0 0;font-size:14px;color:#ffffff;">📅 ${date} | 🔍 Google News + 手動資料</p>
</td></tr>
<tr><td style="padding:24px;">
${['development', 'psychology', 'parenting', 'resources'].map(key => {
    const icons = { development: '🧒', psychology: '💭', parenting: '👪', resources: '📖' };
    const titles = { development: '兒童發展', psychology: '兒童心理', parenting: '教養方法', resources: '資源與經驗' };
...
<a href="https://${GITHUB_PAGES_URL}" style="display:inline-block;padding:14px 32px;background-color:#FF6B9D;color:#ffffff;text-decoration:none;border-radius:8px;font-weight:600;">📖 查看完整報告</a>
</td></tr>
</table>
</body></html>`;

    const mailOptions = {
        from: `"親子教育每日觀察" <${GMAIL_USER}>`,
        to: RECIPIENT_EMAIL,
        subject: `👨‍👩‍👧‍👦 親子教育每日觀察 - ${date}`,
        html: htmlContent
    };
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        h1 { font-size: 2rem; color: #FF6B9D; margin-bottom: 12px; }
        .badge { display: inline-block; background: #FFE5EC; color: #C06C84; padding: 6px 12px; border-radius: 12px; font-size: 0.85rem; margin-top: 8px; }
        ...
        .summary { background: #f8f9fa; padding: 20px; border-radius: 12px; margin-bottom: 24px; border-left: 4px solid #FF6B9D; }
        .point-item { padding: 12px 0 12px 24px; position: relative; }
        .point-item::before { content: '•'; position: absolute; left: 8px; color: #FF6B9D; font-weight: bold; }
        .sources-section { margin-top: 24px; padding-top: 20px; border-top: 2px solid #f1f3f4; }
        .source-link { display: block; margin: 8px 0; padding: 12px 16px; background: #f1f3f4; color: #FF6B9D; text-decoration: none; border-radius: 8px; }
    </style>
</head>
<body>
    <div class="container">
